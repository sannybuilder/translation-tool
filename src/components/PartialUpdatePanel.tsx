...
interface PartialUpdatePanelProps {
  changeTracker: ChangeTracker | null;
  selectedTranslation: string;
  onSubmit?: (changes: TrackedChange[], options: SubmissionOptions) => void;
  refreshTrigger?: number; // Add a trigger to force updates
  isOpen: boolean;
  onClose: () => void;
...
const PartialUpdatePanel: React.FC<PartialUpdatePanelProps> = ({
  ...
  const handleUndoSection = (section: string) => {
    ...
    restoredValues.forEach((originalValue, key) => {
      onUndo(section, key, originalValue);
    });
    ...
    if (restoredValues.size > 0) {
      ...
      setUnsubmittedChanges(changeTracker.getUnsubmittedChanges());
      setStats(changeTracker.getStats());
      // Remove section changes from selected
      const newSelected = new Set(selectedChanges);
      unsubmittedChanges
        .filter(c => c.section === section)
        .forEach(c => newSelected.delete(c.id));
      setSelectedChanges(newSelected);
    }
  };
  ...
  const handleUndoAll = () => {
    ...
    restoredValues.forEach((sectionMap, section) => {
      sectionMap.forEach((originalValue, key) => {
        onUndo(section, key, originalValue);
      });
    });
  ...
  const handleSubmitChanges = async () => {
    // For full file download, we don't need any selections
    if (submissionType === 'download-full') {
      if (onDownloadFullFile) {
        onDownloadFullFile();
      }
      return;
    }
    ...
    const options: SubmissionOptions = {
      type: submissionType,
      format: submissionType === 'copy-clipboard' ? 'ini-snippet' : 'diff',
      includeContext: true,
    };
    ...
    if (submissionType === 'copy-clipboard') {
      ...
      // Create GitHub issue with changes
      const patch = changeTracker.generatePatch(Array.from(selectedChanges), 'ini-snippet');
...
*Submitted using Translation Tool Partial Update feature*`;
    ...
    // History functionality has been removed

    // Mark changes as submitted
    if (submissionType !== 'copy-clipboard') {
      changeTracker.markAsSubmitted(Array.from(selectedChanges));
      setSelectedChanges(new Set());
      setUnsubmittedChanges(changeTracker.getUnsubmittedChanges());
      setStats(changeTracker.getStats());
    }
    ...
    if (onSubmit) {
      onSubmit(changesToSubmit, options);
    }
  };
        ...
        >
          ...
          >
            ...
            <select
              ...
              onChange={(e) => setSubmissionType(e.target.value as SubmissionOptions['type'])}
              style={{
                width: '100%',
                padding: '0.5rem',
                backgroundColor: '#2a2a2a',
            ...
            {Object.entries(groupedChanges).map(([sectionName, changes]) => (
              <div key={sectionName} style={{ marginBottom: '1.5rem' }}>
                {groupBySection && (
                  ...
                  >
                    <div
                      ...
                      onClick={() => handleSelectSection(sectionName)}
                    >
                      <input
                        type="checkbox"
                        checked={changes.every(c => selectedChanges.has(c.id))}
                        onChange={() => {}}
                        style={{ cursor: 'pointer' }}
                      />
                      <h4 style={{ margin: 0, color: '#888', fontSize: '0.9rem' }}>
                        [{sectionName}] ({changes.length})
                      </h4>
                    </div>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleUndoSection(sectionName);
                      }}
                      style={{
                      ...
                      title={`Undo all changes in ${sectionName}`}
                    >
                      Undo Section
                    </button>
                  </div>
                ...
                {changes.map((change) => (
                  ...
                  >
                    <div
                      style={{ display: 'flex', alignItems: 'start', gap: '0.5rem', cursor: 'pointer' }}
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleUndoChange(change.id, change.section, change.key);
                      }}
                      style={{
                        position: 'absolute',
                        top: '0.5rem',
                      ...
                      title={`Undo change to ${change.key}`}
                    >
                      Undo
                    </button>
                  </div>
          ...
          >
            <button
              ...
              style={{
                width: '100%',
                padding: '0.75rem',
                backgroundColor: (submissionType === 'download-full' || selectedChanges.size > 0) ? '#4CAF50' : '#333',
                color: 'white',
                border: 'none',
                borderRadius: '4px',
                fontSize: '1rem',
                fontWeight: 'bold',
                cursor: (submissionType === 'download-full' || selectedChanges.size > 0) ? 'pointer' : 'not-allowed',
...
export default PartialUpdatePanel;
